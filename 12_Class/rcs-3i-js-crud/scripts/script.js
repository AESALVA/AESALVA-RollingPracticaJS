...
const newIdModify =
document.getElementById('idToModify');
const newNameModify =
document.getElementById('nameToModify');
const newPriceModify =
document.getElementById('priceToModify');
const ModifyButton = document.getElementById('modifyButton').addEventListener('click', ()=>upDateNewResults());
...
const updateResults = () => {
  ...
  filtered.forEach(p => {
    ...
    const modbutton = document.getElementById(p.name).addEventListener('click', ()=>findArticle(p)); 
  });
};
...
newPriceModify.value = product.price;
 ...
 const upDateNewResults = () => {
  ...
  products[0].id = newIdModify.value;
  updateResults();
 }
